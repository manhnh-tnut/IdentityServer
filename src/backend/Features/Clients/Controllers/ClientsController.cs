using IdentityServer.Features.Clients.Mappers;
...
namespace IdentityServer.Features.Clients.Controllers;
...
public class ClientsController : ControllerBase
{
    ...
    [HttpGet]
    public async Task<ActionResult<ClientsViewModel>> Get(string searchText, int page = 1, int pageSize = 10)
    {
        var clientsDto = await _clientService.GetClientsAsync(searchText, page, pageSize);
        var clientsViewModel = clientsDto.ToClientViewModel<ClientsViewModel>();
    ...
    public async Task<ActionResult<ClientViewModel>> Get(int id)
    {
        ...
        var clientViewModel = clientDto.ToClientViewModel<ClientViewModel>();
    ...
    {
        var clientDto = client.ToClientViewModel<ClientDto>();
    ...
    [ProducesResponseType(201)]
    [ProducesResponseType(400)]
    public async Task<IActionResult> PostClientClone([FromBody] ClientCloneViewModel client)
    {
        var clientCloneDto = client.ToClientViewModel<ClientCloneDto>();
    ...
    [HttpGet("{id}/Secrets")]
    public async Task<ActionResult<ClientSecretsViewModel>> GetSecrets(int id, int page = 1, int pageSize = 10)
    {
        var clientSecretsDto = await _clientService.GetClientSecretsAsync(id, page, pageSize);
        var clientSecretsViewModel = clientSecretsDto.ToClientViewModel<ClientSecretsViewModel>();

        return Ok(clientSecretsViewModel);
    }

    [HttpGet("Secrets/{secretId}")]
    public async Task<ActionResult<ClientSecretViewModel>> GetSecret(int secretId)
    {
        var clientSecretsDto = await _clientService.GetClientSecretAsync(secretId);
        var clientSecretDto = clientSecretsDto.ToClientViewModel<ClientSecretViewModel>();
    ...
    [ProducesResponseType(201)]
    [ProducesResponseType(400)]
    public async Task<IActionResult> PostSecret(int id, [FromBody] ClientSecretViewModel clientSecretApi)
    {
        var secretsDto = clientSecretApi.ToClientViewModel<ClientSecretsDto>();
    ...
    public async Task<ActionResult<ClientPropertiesViewModel>> GetProperties(int id, int page = 1, int pageSize = 10)
    {
        var clientPropertiesDto = await _clientService.GetClientPropertiesAsync(id, page, pageSize);
        var clientPropertiesViewModel = clientPropertiesDto.ToClientViewModel<ClientPropertiesViewModel>();

        return Ok(clientPropertiesViewModel);
    }

    [HttpGet("Properties/{propertyId}")]
    public async Task<ActionResult<ClientPropertyViewModel>> GetProperty(int propertyId)
    {
        var clientPropertiesDto = await _clientService.GetClientPropertyAsync(propertyId);
        var clientPropertyViewModel = clientPropertiesDto.ToClientViewModel<ClientPropertyViewModel>();
    ...
    [ProducesResponseType(201)]
    [ProducesResponseType(400)]
    public async Task<IActionResult> PostProperty(int id, [FromBody] ClientPropertyViewModel clientPropertyApi)
    {
        var clientPropertiesDto = clientPropertyApi.ToClientViewModel<ClientPropertiesDto>();
    ...
    [HttpGet("{id}/Claims")]
    public async Task<ActionResult<ClientClaimsViewModel>> GetClaims(int id, int page = 1, int pageSize = 10)
    {
        var clientClaimsDto = await _clientService.GetClientClaimsAsync(id, page, pageSize);
        var clientClaimsViewModel = clientClaimsDto.ToClientViewModel<ClientClaimsViewModel>();

        return Ok(clientClaimsViewModel);
    }

    [HttpGet("Claims/{claimId}")]
    public async Task<ActionResult<ClientClaimViewModel>> GetClaim(int claimId)
    {
        var clientClaimsDto = await _clientService.GetClientClaimAsync(claimId);
        var clientClaimViewModel = clientClaimsDto.ToClientViewModel<ClientClaimViewModel>();

        return Ok(clientClaimViewModel);
    }

    [HttpPost("{id}/Claims")]
    [ProducesResponseType(201)]
    [ProducesResponseType(400)]
    public async Task<IActionResult> PostClaim(int id, [FromBody] ClientClaimViewModel clientClaimViewModel)
    {
        var clientClaimsDto = clientClaimViewModel.ToClientViewModel<ClientClaimsDto>();
        ...
        clientClaimViewModel.Id = claimId;

        return CreatedAtAction(nameof(GetClaim), new { claimId }, clientClaimViewModel);
    }
